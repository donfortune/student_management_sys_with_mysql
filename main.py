...
class MainWindow(QMainWindow):
    ...
    def add_data(self):
        connection = mysql.connector.connect(host='localhost', user='root', password='Tangban1.', database='school')
...
class EditStatusBarDialog(QDialog):
    ...
    def update(self):
        connect = mysql.connector.connect(host='localhost', user='root', password='Tangban1.', database='school')
        cursor = connect.cursor()
        cursor.execute("UPDATE students SET name = %s, course = %s, mobile = %s WHERE id = %s", (self.name.text(), self.courses.itemText(self.courses.currentIndex()), self.phone_no.text(), self.get_id))
        connect.commit()
        cursor.close()
        connect.close()
...
class DeleteDialog(QDialog):
    ...
    def delete_student(self):
        ...
        connect =mysql.connector.connect(host='localhost', user='root', password='Tangban1.', database='school')
        cursor = connect.cursor()
        cursor.execute("DELETE from students WHERE id = %s", (get_id, ))
        connect.commit()
        cursor.close()
        connect.close()
        main_window.add_data()
...
class EditDialog(QDialog):
    ...
    def search(self):
        name = self.name.text()
        connection = mysql.connector.connect(host='localhost', user='root', password='xxxxxxxx', database='school')
        cursor = connection.cursor()
